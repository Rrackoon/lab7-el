...
public class CountLessAdminName extends Command {
    public CountLessAdminName() {
        super("count_less_than_group_admin", "вывести количество элементов, значение поля groupAdmin которых меньше заданного");
    }

    @Override
    public Response[] execute(CommandContext context) {
        String name = context.getArgs(); // Получение имени из аргументов команды
